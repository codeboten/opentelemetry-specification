...
`EntryKey` is the name of the Entry. `EntryKey` along with `EntryValue` can be
used to aggregate and group stats, annotate traces and logs, etc.
...
value. Returns the `EntryValue` associated with the given `EntryKey`, or null
if the given `EntryKey` is not present.
...
`EntryKey` entry key for which to set the value.

`EntryValue` entry value to set.

### RemoveCorrelation

To delete a label, the Correlations API provides a function which takes a context
and a key as input. Returns an updated `Context` which no longer contains the selected `EntryKey`.

Required parameter:

`Context` the context from which to remove the correlation.

`EntryKey` entry key to remove.
...
by 'project-id'. Not all entries in a `CorrelationContext` should be propagated and not all entries in a `CorrelationContext`
should be accepted from a remote peer. Hence, `CorrelationContext` propagator must allow specifying an optional
